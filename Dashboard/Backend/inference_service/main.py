...
# from inference_service.repositories import ProductRepository, ImageRepository
...
bucket_name_imgs = "imgs"
bucket_name_models = "models"
bucket_name_prediction_imgs = "predictions"
if not minio_client.bucket_exists(bucket_name_imgs):
    minio_client.make_bucket(bucket_name_imgs)

if not minio_client.bucket_exists(bucket_name_models):
    minio_client.make_bucket(bucket_name_models)

if not minio_client.bucket_exists(bucket_name_prediction_imgs):
    minio_client.make_bucket(bucket_name_prediction_imgs)
...
async def delete_model(model_id: str):
    try:
        ...
        minio_client.remove_object(bucket_name_models, model.model_path.split('/')[-1])
    ...
    try:
        ...
        with open(zip_file_path, "rb") as f:
            minio_client.put_object(
                bucket_name_models,
                file.filename,  # Use the original file name
                f,
...
async def predict_all():
    ...
    try:
        ...
        for image in image_list:
            response = minio_client.get_object(bucket_name_imgs, image.img_url)
            ...
            for model_object in model_list:
                ...
                if skip:
                    ...
                    model_response = minio_client.get_object(bucket_name_models, model_path)
                    model_data = model_response.read()
                    ...
                    small_rect_size = 22 # Size of smaller segments
                    ...
                    for class_idx, label in enumerate(labels):
                        ...
                        with open(f"/tmp/{label}.jpg", "rb") as file:
                            minio_client.put_object(bucket_name_prediction_imgs, key, file, os.path.getsize(f"/tmp/{label}.jpg") )
...
async def predict(img_url: str, img_id: str, model_id: str):
    try:
        ...
        if skip:
            ...
            response = minio_client.get_object(bucket_name_imgs, img_url)
            image_data = response.read()
            response.close()
            response.release_conn()

            # Load the model from MinIO
            model_response = minio_client.get_object(bucket_name_models, model_id)
            model_data = model_response.read()
            model_response.close()
            model_response.release_conn()
